...
function EditSauna({fetchData , ele}) {
const [sauna, setSauna] = useState({ ...ele,id_editing:ele.id, is_available: ele.is_available === 1 || ele.is_available === true});

  const Register = () => {
    const formData = new FormData();
    formData.append('name', sauna.name);
    formData.append('type', sauna.type);
    formData.append('price', parseFloat(sauna.price));
    ...
    axios.post('http://127.0.0.1:8000/api/sauna/edit', formData)
      .then((response) => {
  ...
  return (
    ...
    <div>
      <button type="button" className="btn btn-primary" data-coreui-toggle="modal" data-coreui-target={`#modal-sauna-${ele.id}`}
 onClick={()=>console.log(ele)}
      >
        Edit
      </button>

      <div className="modal fade" id={`modal-sauna-${ele.id}`} tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div className="modal-dialog">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="exampleModalLabel">   Edit Sauna Room Information</h5>
              <button type="button" className="btn-close" data-coreui-dismiss="modal" aria-label="Close"></button>
            </div>
            <div className="modal-body">
              <div style={{ display: 'flex', flexDirection: 'column',textAlign:'start', justifyItems:'start',padding: '20px', width: '100%' }}>
                <div className="mb-3">
                ...
                <div className="mb-3">
                  ...
                  <div className="form-check form-check-inline">
                    <input
                      type="radio"
                      name={`is_available-${sauna.id_editing}`}
                      value="true"
